...
namespace gestion_de_formations_d_une_école
{
    public partial class Examen : Form
    {
        ...
        public int count()
        {
            int cpt;
            p.connecter();
            p.cmd = new System.Data.SqlClient.SqlCommand("select count(code_stagiaire) from Examen where code_stagiaire='" + comboBox1.SelectedValue + "' AND code_mod='"+comboBox2.SelectedValue+"'", p.con) ;
        ...
        public bool ajouter()
        {
            ...
            if (count() == 0)
            {
                ...
                p.cmd = new System.Data.SqlClient.SqlCommand("insert into Examen values ('" + comboBox1.SelectedValue+ "','" + comboBox2.SelectedValue + "','" + dt1 + "','" + textBox1.Text + "')", p.con);
        ...
        public bool modifier()
        {
            if (count() != 0)
            {
                p.connecter();
                DateTime dt1 = DateTime.Parse(dateTimePicker2.Text);
                p.cmd.CommandText = "update Examen set DateExamen='" + dt1 + "',Note='" +textBox1.Text  + "' where code_stagiaire='" + comboBox1.SelectedValue + "' AND code_mod='" + comboBox2.SelectedValue + "''";
                p.cmd.ExecuteNonQuery();
                p.deconnecter();
                return true;
            }
        ...
        {
            p.connecter();
            p.cmd = new System.Data.SqlClient.SqlCommand("select * from Examen", p.con);
            p.dr = p.cmd.ExecuteReader();
